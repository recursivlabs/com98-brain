...
import { ORG_ID, PROJECT_ID, createAuthedSdk, anonSdk } from './recursiv';
...
// 4: Project Membership rollout — api keys are now project-scoped, not
// org-scoped. Existing brain:* keys would be ignored anyway since this is a
// fresh org, but bumped for consistency with kempt/rolodex.
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const verifyOtp = React.useCallback(async (email: string, otp: string) => {
    const result = await anonSdk.auth.verifyOtpAndCreateKey(
      { email, otp },
      { name: 'com98-brain-' + Date.now(), scopes: [...API_KEY_SCOPES], projectId: PROJECT_ID },
    );
